...
class OsuAPI {
  ...
  constructor(osuToken) {
    if (!osuToken) console.log("It seems that osu token are not inserted, skipping");
  ...
  getUser(name, mode = "standard") {
    ...
    return new Promise((resolve, reject) => {
      this.#_request("/get_user", `&u=${name}&m=${OsuGameModes[mode]}`)
        .then((res) => {
  ...
  getUserRecent(name, mode = "standard") {
    if (!name || typeof name !== "string") throw new TypeError("No username specified");
    return new Promise((resolve, reject) => {
      this.#_request("/get_recent_user", `&u=${name}&m=${OsuGameModes[mode]}`)
        .then((res) => {
          res.data[0]["mode"] = OsuGameModesName[mode];
          resolve(res.data[0]);
        })
  ...
  getUserBest(name, mode = "standard") {
    if (!name || typeof name !== "string") throw new TypeError("No username specified");
    return new Promise((resolve, reject) => {
      this.#_request("/get_user_best", `&u=${name}&m=${OsuGameModes[mode]}`)
        .then((res) => {
          res.data[0]["mode"] = OsuGameModesName[mode];
          resolve(res.data[0]);
        })
